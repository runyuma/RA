...
def display_image(path_or_array, size=(10, 10)):
  if isinstance(path_or_array, str):
    image = np.asarray(Image.open(open("./", "rb")).convert("RGB"))
  else:
    image = path_or_array
...
def vild(session,clip_model,image_path, category_name_string, params, plot_on=True, prompt_swaps=[], verbose=False):
  #################################################################
  ...
  for idx,anno_idx in enumerate(indices[0:int(rescaled_detection_boxes.shape[0])]):
    ...
    box_sizes = [int(rescaled_detection_boxes[anno_idx][0]),
                 int(rescaled_detection_boxes[anno_idx][1]),
